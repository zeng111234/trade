...
const WEIGHTS = {
  ...
  feePenalty: -1.5,      // 费率惩罚系数 (feeRate * this)
  rotationPenalty: -2.0, // 连续推荐惩罚 (per day)
  suspended: -999
};

function scoreFund(fund, indicators, historyContext, recentPicks) {
...
async function allocateDynamic(budget, funds, config) {
  ...
  var scored = fundDataPairs.map(({ fund, indicators }) => {
    ...
    var result = scoreFund(fund, indicators, histCtx, recentPicksMap);
    return { ...fund, ...result };
  });
