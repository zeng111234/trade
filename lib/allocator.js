...
function filterAvailable(funds) {
  return funds.filter(f => f.status === "active" && f.dailyLimit > 0);
}
...
function allocate(budget, funds, strategy) {
  if (!strategy) strategy = Strategy.SCARCE_FIRST;
  const available = filterAvailable(funds);
  const suspended = funds.filter(f => f.status !== "active" || f.dailyLimit <= 0);
  let allocations;
  switch (strategy) {
